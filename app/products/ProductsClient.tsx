      ...
      <div className="min-w-0">
        ...
        {filtered.length ? <div className="mt-8 grid gap-6 sm:grid-cols-2 xl:grid-cols-3">{filtered.map((product) => (
          <article key={product.id} className="group flex flex-col overflow-hidden border border-black/10 bg-white transition-[transform,box-shadow] hover:-translate-y-1 hover:shadow-[0_20px_50px_rgba(0,0,0,0.10)]">
            <div className="relative aspect-square w-full overflow-hidden bg-black/[0.025]">{product.primaryImg ? <Image src={product.primaryImg} alt={product.title} fill sizes="(min-width: 1280px) 25vw, (min-width: 640px) 45vw, 100vw" className="object-cover object-center transition-transform duration-500 group-hover:scale-[1.035]" /> : <div className="flex h-full flex-col items-center justify-center gap-3 text-black/40"><ImageIcon className="h-9 w-9 stroke-[1.25]" /><span className="text-xs font-semibold uppercase tracking-[0.13em]">Ürün Görseli Yakında</span></div>}</div>
