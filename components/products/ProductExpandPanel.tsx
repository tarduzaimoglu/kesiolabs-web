...
function pickQtyRules(product: Product) {
  ...
  const min = minFromText ?? CART_MIN_QTY;

  const max = 2000; // ✅ sabit 2000

  let step = CART_STEP;
  if (!Number.isFinite(step) || step <= 0) step = 1; // istersen 1 bırak
...
export function ProductExpandPanel({
  ...
  // ✅ Adet input: serbest yazma + min/max uyarısı
          ...
          <div className="w-full text-black">
            ...
            <div className="mt-6 grid w-full grid-cols-2 gap-4">
              ...
              <button
                ...
                disabled={!qtyValid}
                className={[
                  "rounded-xl px-4 py-3 text-sm font-medium text-white transition",
                  BLUE,
                  !qtyValid ? "opacity-50 cursor-not-allowed" : "",
                ].join(" ")}
              >
                Sepete Ekle
              </button>
